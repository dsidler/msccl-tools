    ...
    for r1 in range(ngpus):
        for tb in range(numTbs):
            for r2 in range(ngpus):
                if r1 != r2:
                    index = offset + (tb * sizePerTb) + (r2 * sizerPerRank)
                    c = chunk(gpuIds[r1], Buffer.input, index, sizerPerRank)
                    c.signal(gpuIds[r2], Buffer.input, index, sendtb=tb)

    for r1 in range(ngpus):
        for tb in range(numTbs):
            for r2 in range(ngpus):
                if r1 != r2:
                    index = offset + (tb * sizePerTb) + (r1 * sizerPerRank)
                    c = chunk(gpuIds[r1], Buffer.input, index, sizerPerRank)
                    c.wait(gpuIds[r2], Buffer.input, index, recvtb=tb)
    ...
    for r in range(ngpus):
        for tb in range(numTbs):
            ...
            for peer in range(ngpus):
                if peer != r:
                    c.reduce(chunk(gpuIds[peer], Buffer.input, chunkIndex, sizerPerRank), recvtb=tb)
        ...
        for tb in range(numTbs):
            for peer in range(ngpus):
                if peer != r:
                    ...
                    c.signal(gpuIds[peer], Buffer.input, index, sendtb=tb)
        ...
        for tb in range(numTbs):
            for peer in range(ngpus):
                if peer != r:
                    ...
                    c = chunk(gpuIds[r], Buffer.input, index, sizePerRank)
                    c.wait(gpuIds[peer], Buffer.input, index, recvtb=tb)
                    c.get(gpuIds[peer], Buffer.input, index, recvtb=tb)
